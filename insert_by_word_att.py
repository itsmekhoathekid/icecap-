...
def insert(cap, word_att, ner_dict, related_words, key, context=False):
    sen = []
    names = []
...
def main(params):
    ...
    if dataset == 'breakingnews':
        ...
        id_to_key = {h['image_id']: h['image_id'].split('_')[0] for h in output}
    ...
    for h in tqdm.tqdm(output):
        ...
        ref_name.append(test_compact[index]['sentences'][0]['name'])
        ...
        if len(related_sentences) > 0:
            ...
            sen, inserted_name = insert(cap, sorted_word_att_locs, ner_dict, related_words, key ,context=False)
        else:
            inserted_name = []
            related_non += 1
            sen = cap
    ...
    if True:
        json.dump(hypo, open(template_path.replace('.json', '_full.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
...
if __name__=='__main__':
    ...
    parser.add_argument('--template_path', type=str,
                        default='/data/npl/ICEK/ICECAP/icecap-/ViWiki_output/ViWiki_test.json',
