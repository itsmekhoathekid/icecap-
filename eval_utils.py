...
#                 arr = np.stack(match_seqprobs, axis=0)
#                 match_seqprobs = arr.transpose(1, 0, 2)
#                 print(match_seqprobs.shape)
#                 # max_len = max(arr.shape[0] for arr in match_seqprobs)
#                 # word_len = match_seqprobs[0].shape[1]
#                 # batch = len(match_seqprobs)
#                 # padded = np.zeros((batch, max_len, word_len), dtype=match_seqprobs[0].dtype)
#                 # for i, arr in enumerate(match_seqprobs):
#                 #     L = arr.shape[0]
#                 #     padded[i, :L, :] = arr
#                 # match_seqprobs = padded
#                 print("calculated match_seqprobs")
#             else:
#                 print("go B_B")
...
#                 # match_seqprobs_sent = np.array(match_seqprobs[k, :sen_length, :])  # sent_len * word_length
...
#         print('match precision:%.3f' % (float(match_all_true_num) / match_all_target_num if match_all_target_num != 0 else 0))
